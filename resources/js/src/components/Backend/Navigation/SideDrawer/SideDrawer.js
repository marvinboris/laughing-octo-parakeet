...
import { faTachometerAlt, faUserTie, faTasks, faCog, faBoxOpen, faMoneyBillWaveAlt, faCity, faBuilding, faUserTag, faTools, faUser, faUserCog, faLanguage, faMoneyBill, faBell, faWrench } from '@fortawesome/free-solid-svg-icons';
    ...
    const {
        pages: {
            backend: {
                sidebar: {
                    user, admin,
                    menu: { dashboard, admins, users, roles, features, customers, cities, quarters, languages, methods, canal, eneo, camwater, expenses, cms: cms_, notifications: notifications_, settings, }
                }
            }
        }
    } = cms;
    ...
    switch (role) {
        case 'user':
            ...
            sideDrawerItems = <>
                ...
                {sideDrawerItem(false, null, customers, faUserTie, "/user/customers", false, [
                    { link: '/categories/add', text: customers.categories.add },
                    { link: '/categories', text: customers.categories.index },
                ])}
                {sideDrawerItem(false, null, cities, faCity, "/user/cities")}
                {sideDrawerItem(false, null, quarters, faBuilding, "/user/quarters")}
                {sideDrawerItem(false, null, languages, faLanguage, "/user/languages")}
                {sideDrawerItem(false, null, methods, faMoneyBill, "/user/methods")}
                {sideDrawerItem(true, null, canal, faBoxOpen, "/user/canal", [
                    { link: '/formulae', text: canal.formulae },
                    { link: '/subscriptions', text: canal.subscriptions.index },
                    { link: '/subscriptions/add', text: canal.subscriptions.add },
                    { link: '/case', text: canal.case },
                ])}
                {sideDrawerItem(true, eneo.title, null, faTasks, "/user/eneo")}
                {sideDrawerItem(true, camwater.title, null, faTasks, "/user/camwater")}
                {sideDrawerItem(false, null, expenses, faMoneyBillWaveAlt, "/user/expenses")}
                ...
                <SideDrawerItem id={notifications_} sideDrawerToggle={toggle} select={selectItem} selected={selectedItem} icon={faBell} href="/user/notifications">
                    {notifications_}{' '}
                    <Badge color="orange" className="position-relative rounded-circle text-x-small text-700 d-inline-flex justify-content-center align-items-center" style={{ width: 18, height: 18, top: -7 }}><b className="text-white">{notifications.length}</b></Badge>
                </SideDrawerItem>
        ...
        case 'admin':
            sideDrawerItems = <>
                ...
                {sideDrawerItem(true, null, customers, faUserTie, "/admin/customers", false, [
                    { link: '/categories/add', text: customers.categories.add },
                    { link: '/categories', text: customers.categories.index },
                ])}
                {sideDrawerItem(true, null, cities, faCity, "/admin/cities")}
                {sideDrawerItem(true, null, quarters, faBuilding, "/admin/quarters")}
                {sideDrawerItem(true, null, languages, faLanguage, "/admin/languages")}
                {sideDrawerItem(true, null, methods, faMoneyBill, "/admin/methods")}
                {sideDrawerItem(true, null, canal, faBoxOpen, "/admin/canal", [
                    { link: '/formulae', text: canal.formulae },
                    { link: '/subscriptions', text: canal.subscriptions.index },
                    { link: '/subscriptions/add', text: canal.subscriptions.add },
                    { link: '/case', text: canal.case },
                ])}
                {sideDrawerItem(true, eneo.title, null, faTasks, "/admin/eneo")}
                {sideDrawerItem(true, camwater.title, null, faTasks, "/admin/camwater")}
                {sideDrawerItem(true, null, expenses, faMoneyBillWaveAlt, "/admin/expenses")}
