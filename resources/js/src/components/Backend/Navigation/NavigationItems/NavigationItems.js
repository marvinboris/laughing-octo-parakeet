...
export default ({ data, sidedrawerToggle, logoutHandler, role, messages = [], notifications = [], date: { weekDay, day, month, year }, clock: { hours, minutes, seconds }, cms }) => {
    ...
    if (cms) {
        ...
        notificationsContent = <>
            <UncontrolledDropdown inNavbar>
                <DropdownToggle nav className="p-0">
                    ...
                    <Badge color="yellow" className="position-absolute rounded-circle d-inline-flex justify-content-center align-items-center" style={{ width: 20, height: 20, transform: 'translate(-30px, -5px)', zIndex: 2 }}>{messages.length}</Badge>
                </DropdownToggle>
        ...
        logoutContent = <>
            ...
            <ModalBody className="text-center">
                <p>{sure_logout}?</p>
                <div>
                    <Button color="orange" onClick={logoutHandler}>{logout} <FontAwesomeIcon icon={faPowerOff} fixedWidth /></Button>{' '}
                    <Button color="brown" onClick={toggle}>{close_} <FontAwesomeIcon icon={faTimes} fixedWidth /></Button>
                </div>
            </ModalBody>
        </>;
    }
    ...
    return <div className="px-3 bg-darkhead flex-fill d-flex align-items-center text-white text-large position-relative" style={{ height: 70 }}>
