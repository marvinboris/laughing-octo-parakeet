...
export default ({ icon, addon, onChange, className = '', name, type = 'text', required, readonly, placeholder, value = '', defaultValue = '', validation = {}, append, children }) => {
    ...
    if (type === 'datetime-local') content = <DateTimeInput id={name} name={name} required={required} readOnly={readonly} className={"w-100 d-flex text-small h-100"} value={value} defaultValue={defaultValue} placeholder={placeholder} />
        ...
        {children ?
            ...
            :
            <Input valid={touched && checkValidity(value, validation)} invalid={touched && !checkValidity(value, validation)} onChange={inputChangedHandler} type={type} id={name} name={name} required={required} readOnly={readonly} value={value} defaultValue={defaultValue} className={"text-small h-100 px-4 py-3"} placeholder={placeholder} />
        }
