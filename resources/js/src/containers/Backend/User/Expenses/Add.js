...
class Add extends Component {
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { expenses: { title, add, index, form } } } }
                }
            },
            auth: { data: { name, role: { features } } },
            backend: { expenses: { loading, error, message, methods, expendables } }
        } = this.props;
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faMoneyBillWaveAlt} title={add} list={index} link="/user/expenses" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer' }} onClick={this.fileUpload}>
                                    {proof && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb main={add} icon={faMoneyBillWaveAlt} />
...
const mapDispatchToProps = dispatch => ({
    get: () => dispatch(actions.getExpensesInfo()),
