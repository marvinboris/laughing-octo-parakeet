...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features_.find(f => f.prefix === 'features');
        ...
        const data = features.map(feature => {
            return updateObject(feature, {
                ...
                action: <div className="text-center">
                    ...
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/features/${feature.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(feature.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
