...
class Add extends Component {
    ...
    submitHandler = async e => {
        e.preventDefault();
        await this.props.patch(this.props.match.params.customerId, e.target);
    }
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { customers: { title, edit, index, form } } } }
                }
            },
            backend: { customers: { loading, error, message, cities, customer } }
        } = this.props;
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faUserTie} title={edit} list={index} link="/admin/customers" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: photo && `url("${photo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {photo && (photo !== customer.photo) && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb items={[{ to: '/admin/customers', content: index }]} main={edit} icon={faUserTie} />
...
const mapDispatchToProps = dispatch => ({
    get: id => dispatch(actions.getCustomer(id)),
